...
- **Rate limits**: Groq free tier allows ~8k tokens per minute *per model*. With a ~4.7k-token prompt that is about one request per model per minute, so `createStream` falls back to the next model on 429/413 (`maxRetries: 0` so the switch is immediate) and the tool confirmation is written without a second model pass. If traffic grows, upgrade the Groq account (Dev tier) or shrink the prompt further.
